...
## 1. build.gradle에 추가한 Security 의존성 5종
...
```
- 클래스 상단에 Spring Security 개념 총정리 주석을 남겨둠. 인증 vs 인가, DispatcherServlet 필터 체인 위치, AuthenticationFilter → AuthenticationManager → AuthenticationProvider → UserDetailService 흐름을 정리한 내용임
- `csrf.disable()`은 학습 단계라 CSRF 방어를 꺼둔 상태. 주석에는 "일반 보안 = csrf.disable()"이라고만 메모돼 있음. 운영 전환 시 재검토 필요
- 경로별 권한: `/`, `/member/**`는 전체 허용, `/admin/**`는 `ROLE_ADMIN` 필요. 다만 바로 아래 `.anyRequest().permitAll()`이 있어 사실상 전체 허용과 동일한 효과임. 위 두 규칙이 뒤 규칙에 가려지는 형태는 아니지만, 세부 경로 외 나머지가 전부 열려 있어 인가 정책이 아직 느슨함
- `rememberMe().key("my-secret-key")`는 remember-me 토큰 서명용 키가 **소스에 문자열 그대로 하드코딩**된 상태. 지금은 튜토리얼용 더미 값이지만, Day07~09에서 지켜온 `${환경변수}` 분리 원칙을 여기에도 적용해야 함
- `logout()`에서 `remember-me` / `JSESSIONID` 쿠키를 함께 삭제하도록 세팅

---

## 3. 인증 관련 Bean 3종 — 전부 `null` 반환하는 미완성 스텁
...
```
- 둘 다 `@Component`로 등록만 해두고 본문은 아직 구현되지 않은 상태. `SecurityConfig`의 `successHandler`/`failureHandler`에는 이미 연결돼 있어, 인터페이스 구현체를 먼저 배치하고 로직은 다음에 채우는 순서로 진행 중
...
- 둘 다 `@Data`만 붙은 순수 VO로, Day09의 다른 VO들처럼 아직 `@Entity` 전환 전 단계
- `JdbcUserDetailsManager`가 정상 구현되면 이 두 테이블 구조(`MEMBER`/`AUTHORITY`)를 기반으로 `users-by-username-query`/`authorities-by-username-query` SQL을 채워 넣어야 함

---

## 6. 로그인 폼·회원가입 스텁·header 권한 표시 준비
...
```
- `input name`이 `SecurityConfig`의 `.usernameParameter("userid")` / `.passwordParameter("userpwd")`와 정확히 일치함. 커스텀 파라미터명을 쓸 때 폼 필드명도 반드시 맞춰야 함
- 다만 `<form>` 태그와 `action`/`method`가 빠져 있어 아직 실제 제출은 되지 않는 정적 마크업 상태. 버튼이 `type="submit"`이어도 감싸는 폼이 없음. 다음 작업에서 `<form th:action="@{/member/login_process}" method="post">`를 추가해야 함
...
```
- `<title>` 외 내용이 전혀 없는 빈 스텁으로, 회원가입 화면 자리만 잡아둔 상태

### `header.html`에 추가한 sec 네임스페이스
```html
...
- 쉐프 메뉴를 기존 위치에서 빼서 `sec:authorize` 속성과 함께 주석 처리한 채로 옮김. 로그인 권한에 따라 노출 여부를 제어할 예정이지만 `sec:authorize=""` 값이 비어 있어 아직 조건이 채워지지 않음(예: `sec:authorize="isAuthenticated()"`)
