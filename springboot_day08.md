# 📘 Spring Boot Day 08 — Recipe MyBatis 전환 + CDN 방식 Pinia + 환경변수 기반 Docker 배포

> ⚠️ 이 문서에서 IP, DB 계정, 컨테이너 ID 등은 모두 `<...>` 형태로 마스킹 처리했습니다. 실제 값은 각자 환경에 맞게 채워 넣으세요.
...
```
- `RecipeDetailVO`는 Day05에서 정리했던 `RecipeDetail` Entity와 **컬럼 구성이 완전히 동일** — 처리 방식만 JPA → MyBatis로 바뀐 것
- `ChefVO`, `FoodVO`도 같은 방식으로 VO 전환됨 (`FoodMapper`는 아직 내용 없이 껍데기만 생성된 상태)

### RecipeMapper — 어노테이션 SQL + XML SQL 혼합
```java
@Mapper
@Repository
...
```
- **목록/카운트처럼 조건(WHERE)이 복잡한 쿼리는 XML**에, **단순한 UPDATE/SELECT는 `@Update`/`@Select` 어노테이션**으로 소스 코드 안에 바로 작성 — 쿼리 복잡도에 따라 두 방식을 섞어 쓰는 설계

### recipe-mapper.xml — 재사용 SQL 조각(`<sql>`) 활용
```xml
...
```
- **`INTERSECT`** 로 두 테이블에 공통으로 존재하는 `no`만 필터링하는 서브쿼리를 `<sql id="where-no">`로 빼서 **목록 조회 + 카운트 조회 양쪽에서 `<include>`로 재사용** — 쿼리 중복 제거 패턴
...
### Service/RestController — 반환 구조는 이전과 동일 패턴 유지
```java
// RecipeServiceImpl
...
```
> Vue 레포(VueStudy) Day01에서 정리했던 "list_vue/detail_vue가 백엔드에 없다"는 메모가 **오늘 자료로 해소됨** — 두 레포 진행 상황이 맞춰진 지점

### RecipeController — 화면 진입만 담당
```java
...
### th:inline="javascript" — 서버 값을 JS로 안전하게 전달
```html
...
```
- Day05에서 썼던 `const chef_name='[[${chef}]]'`(문자열 따옴표로 감싸는 방식)와 달리, **`th:inline="javascript"` + `/*[[${}]]*/` 주석 방식**은 **숫자/객체 등 타입을 그대로 유지**할 수 있어서 문자열이 아닌 값에 더 적합
- 화면단 페이징: `store.range`, `store.move(i)`처럼 **페이지 배열 생성과 이동 로직 자체를 Pinia store 안으로 옮겨서** 화면(HTML)은 `store.xxx`를 호출만 하는 구조로 단순화됨 (Day04의 `range()` 함수가 컴포넌트에 있었다면, 오늘은 store로 이동)

---

## 3. Docker 배포 — 환경변수 주입 3단계 실습
...
```
- 기존 컨테이너 정리는 지금까지와 동일하게 `docker stop` → `docker rm` 순서로 진행
...
```
> **포인트**: `-e DB_URL`처럼 **값을 직접 안 쓰고 변수명만 적으면**, 현재 쉘 세션에 로드돼 있는 환경변수 값을 그대로 컨테이너에 전달함 → `docker run` 명령 자체에는 실제 비밀번호가 노출되지 않음 (`.env`를 `source` 하거나 `export`한 뒤 실행한다는 전제)
...
> ⚠️ **오늘 배운 교훈(사용자 메모)**: "아이피 및 API는 항상 가릴 것" — `.env`나 `/etc/environment`에 넣는 것과 별개로, **학습 정리/커밋/캡처 어디에도 실제 IP·계정정보·API 키를 그대로 남기지 않기**. `.env` 파일 자체도 `.gitignore`에 반드시 추가해서 레포에 올라가지 않도록 확인.
