...
| Day 17 | `SpringPiniaProject_2` 댓글 알림을 Kafka 경유 구조로 전환(`NoticeProducer`/`NoticeConsumer`, Controller의 직접 STOMP 전송을 Kafka Producer 발행으로 교체), Jsoup 크롤링(`DataCollection`)으로 실시간 랭킹 수집 스케줄러 완성(1분 주기), 댓글 화면을 테이블 목록에서 카드형 UI로 리디자인 / AWS EC2 인스턴스에 CI/CD 배포 서버 신규 구축(자바21+Docker+Docker Compose+Kafka 컨테이너, SSH 키 인증 등록, 보안 그룹 포트 개방, 배포 후 리소스 정리) | [📄 보기](./springboot_day17.md) |
...
| Day 21 | 신규 프로젝트 `SpringPostgreProject` 초기 스캐폴딩(PostgreSQL+MyBatis 조합, `@Select` 어노테이션 매퍼, Controller-Mapper 최소 구성), Thymeleaf `th:each`+인라인 표현식(`[[...]]`)으로 목록 출력, Git 브랜치 전략(main/develop/feature) 신규 도입+`gh pr create`/`merge`로 PR 경유 병합 흐름 정리 / Oracle+PostgreSQL **다중 DataSource** 연동(이중 SqlSessionFactory+`@Qualifier`), Oracle→PostgreSQL 레시피 데이터 이관 서비스, Spring AI Embedding으로 레시피 벡터화 후 `pgvector`에 저장하는 구조 신규 구현 / `SpringRecipeAIProject` Jenkins 파이프라인을 빌드~Docker 배포까지 확장(`.env` 생성+DockerHub 로그인/푸시+컨테이너 재배포), 재료 기반 레시피 추천 전체 흐름(임베딩 검색→pgvector 유사도 검색→재료 충족률 계산) 설계 | [📄 보기](./springboot_day21.md) |
| Day 22 | `SpringRecipeAIProject` Jenkins 배포 파이프라인을 `docker run` 수동 제어에서 `docker compose down/pull/up -d` 기반으로 전환(이미지명은 `DOCKER_IMAGE` 환경변수로 통일), `docker-compose.yml` 신규 작성(`restart: always`+`.env` 주입), `post { success/failure }`로 배포 결과 분기 처리+`Container Check` 단계 추가, DockerHub 로그인 명령의 파이프(`\|`) 누락 버그 수정 | [📄 보기](./springboot_day22.md) |
